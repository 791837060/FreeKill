...
Rectangle {
    ...
    property string wordMp3: ""
    property string wordMp3Zh: ""
    property var resolveAudioPath: function(p) { return p; }
    property var dialogHost: null
    property int lastZhSoundMs: 0

    function playZhHintSound() {
        var path = resolveAudioPath(wordMp3Zh);
        if (!path)
            return;
        var now = Date.now();
        if (now - lastZhSoundMs < 250)
            return;
        lastZhSoundMs = now;
        Backend.playSoundWav(path);
    }
                     ...
                     TapHandler {
                        ...
                        onTapped: {
                            ...
                            if (input1.text.length % 3 === 1)
                                virtualKeyboard.playZhHintSound();
                        }
                    }
                }
            }
                ...
                //}
                 TapHandler {
                        ...
                        onTapped: {
                            input1.text = ""
                            if (dialogHost) {
                                dialogHost.resetTypingTrack()
                                dialogHost.resetWordSessionState()
                                dialogHost.updateAnkiStatus()
                            }
                        }
                    }
            }
