...
if not insertWordListBack1 then
  dofile "lua/lunarltk/server/events/word_banks.lua"
end
...
local function splitWord(word)
  ...
  for _ = 1, numToAdd do
    ...
    table.insert(charTable, availableLetters[randomIndex])
    table.remove(availableLetters, randomIndex)
  end
  shuffleTable(charTable)
  local connectorString = "_xxxxxx_"
  local subStr = ""
  for i = 1, #charTable do
    subStr = subStr .. charTable[i] .. connectorString
  end
  return subStr
end
