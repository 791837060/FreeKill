...
local WordTest = require "lunarltk.server.events.word_test"
...
function Phase:main()
  ...
  switch(player.phase, {
    ...
    [Player.RoundStart] = function()
      WordTest.playSoundIfAllowed(room, WordTest.currentWordSound)
    end,
    [Player.Start] = function()
      WordTest.beginTurn(player)
      WordTest.currentWordSound = WordTest.showWord(player, room)
      WordTest.playSoundIfAllowed(room, WordTest.currentWordSound)
    end,
    ...
    [Player.Play] = function()
      ...
      while not player.dead do
        ...
        if wordTestEnabled then
          dat.timeout = WordTest.PLAY_PHASE_TIMEOUT
        end
        ...
        if result == "" then
          if WordTest.canShowWord(player) then
            WordTest.currentWordSound = WordTest.showWord(player, room)
          end
          break
        end
