...
Fk:loadTranslationTable{
  ...
  "若如此做，该角色的下回合结束阶段开始时，其可以依次视为对你使用等量的【杀】。",
...
juanxia:addEffect(fk.EventPhaseStart, {
  ...
  on_cost = function(self, event, target, player, data)
    ...
    if mark == 0 then
      mark = table.filter(room:getUniversalCards("t"), function(id)
...
juanxia:addEffect(fk.EventPhaseStart, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target.phase == Player.Finish and
      not player.dead and not target.dead and target:getMark("@juanxia") > 0 and
  ...
  can_refresh = function(self, event, target, player, data)
    return target == player and player.phase == Player.Finish and
      (player:getMark("@juanxia") > 0 or player:getMark("juanxia_record") ~= 0)
  end,
