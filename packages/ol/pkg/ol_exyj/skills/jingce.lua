...
Fk:loadTranslationTable{
  ...
  ["@[suits]ol_ex__jingce-turn"]="精策",
  ["$ol_ex__jingce1"] = "此地薄弱，当加强防御。",
  ["$ol_ex__jingce2"] = "诸葛小计，吾早已预料。",
}
...
jingce:addEffect(fk.CardUsing, {
  can_trigger = function (self, event, target, player, data)
    return target == player and player:hasSkill(jingce.name) and
      ...
      not table.contains(player:getTableMark("@[suits]ol_ex__jingce-turn"), data.card.suit)
  end,
  on_cost = Util.TrueFunc,
  on_use = function (self, event, target, player, data)
    local room = player.room
    room:addPlayerMark(player, MarkEnum.AddMaxCardsInTurn, 1)
    room:addTableMark(player, "@[suits]ol_ex__jingce-turn", data.card.suit)
  end,
})
