  ...
  can_trigger = function(self, event, target, player, data)
    if data.extra_data and data.extra_data.zixi and not player.dead then
      ...
      if use.from == player then
        local card = Fk:cloneCard(data.card.name, data.card.suit, 0)
...
zixi:addEffect(fk.EventPhaseProceeding, {
  ...
  on_refresh = function (self, event, target, player, data)
    ...
    local cards = table.filter(player:getCardIds("j"), function (id)
      return not (player:getVirtualEquip(id) and player:getVirtualEquip(id).name == "zixi_trick")
    end)
    while #cards > 0 do
      ...
      if table.contains(player:getCardIds("j"), cid) then
