local tianxiang = fk.CreateSkill{
  name = "m_ex__tianxiang",
}

Fk:loadTranslationTable {
  ["m_ex__tianxiang"] = "天香",
  [":m_ex__tianxiang"] = "当你受到伤害时，你可以弃置一张<font color='red'>♥</font>手牌并选择一名其他角色。你防止此伤害并选择一项："..
  "1.伤害来源对其造成1点伤害，其摸X张牌（X为其已损失的体力值且至多为5）；2.令其失去1点体力，其获得牌堆或弃牌堆中你以此法弃置的牌。",

  ["#m_ex__tianxiang-choose"] = "天香：弃置一张<font color='red'>♥</font>手牌并选择一名其他角色",
  ["#m_ex__tianxiang-choice"] = "天香：选择一项令 %dest 执行",
  ["m_ex__tianxiang_damage"] = "令其受到1点伤害并摸已损失体力值的牌",
  ["m_ex__tianxiang_loseHp"] = "令其失去1点体力并获得你弃置的牌",

  ["$m_ex__tianxiang1"] = "盈盈一笑，娇花照水。",
  ["$m_ex__tianxiang2"] = "玉容花貌难自弃。",
}
...
tianxiang:addEffect(fk.DetermineDamageInflicted, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local choices = {"m_ex__tianxiang_loseHp"}
    if data.from and not data.from.dead then
      table.insert(choices, 1, "m_ex__tianxiang_damage")
    end
    local choice = room:askToChoice(player, {
      choices = choices,
      skill_name = tianxiang.name,
      prompt = "#m_ex__tianxiang-choice::"..to.id,
    })
    if choice == "m_ex__tianxiang_loseHp" then
