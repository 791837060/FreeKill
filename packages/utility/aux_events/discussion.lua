...
--- 议事牌展示前
---@class Utility.DiscussionCardsDisplaying: Utility.DiscussionTE
Utility.DiscussionCardsDisplaying = Utility.DiscussionTE:subclass("Utility.DiscussionCardsDisplaying")
--- 议事牌展示后
...
Fk:addGameEvent(Utility.DiscussionEvent, nil, function (self)
  ...
  logic:trigger(Utility.DiscussionCardsDisplaying, nil, discussionData)
  ...
  for _, p in ipairs(discussionData.tos) do
    ...
    if discussionData.results[p].toCards then
      p:filterHandcards()
      local opinion = ""
      for _, id in ipairs(discussionData.results[p].toCards) do
        if opinion == "" then
          opinion = Fk:getCardById(id):getColorString()
        elseif opinion ~= Fk:getCardById(id):getColorString() then
          opinion = "nocolor"
        end
      end
      discussionData.results[p].opinion = opinion
