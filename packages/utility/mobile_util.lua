...
--- SkillChargeChangeData 蓄力值变更的数据
---@class SkillChargeChangeDataSpec
---@field public num integer @ 蓄力点变更数值

---@class Utility.SkillChargeChangeData: SkillChargeChangeDataSpec, TriggerData
Utility.SkillChargeChangeData = TriggerData:subclass("SkillChargeChangeData")

--- 蓄力值变更 TriggerEvent
---@class Utility.SkillChargeChangeTE: TriggerEvent
---@field public data Utility.SkillChargeChangeData
Utility.SkillChargeChangeTE = TriggerEvent:subclass("SkillChargeChangeEvent")

--- 整肃奖励后
---@class Utility.SkillChargeChanged: Utility.SkillChargeChangeTE
Utility.SkillChargeChanged = Utility.SkillChargeChangeTE:subclass("Utility.SkillChargeChanged")
...
Utility.skillCharged = function(player, num, max)
  ...
  if changeNum ~= 0 then
    room.logic:trigger(Utility.SkillChargeChanged, player, { num = changeNum })
  end
end
...
Utility.doStrategy = function(room, from, to, from_choices, to_choices, skillName, msg_type, prompts)
  ...
  local req = Request:new({from, to}, "AskForChoices")
  req:setData(from, { from_choices, from_choices, {1, 1}, false, skillName, prompts[1], true, true })
  req:setData(to, { to_choices, to_choices, {1, 1}, false, skillName, prompts[2], true, true })
  req:setDefaultReply(from, room:tableRandomPick(from_choices, 1))
  req:setDefaultReply(to, room:tableRandomPick(to_choices, 1))
  req.focus_text = skillName
  req.receive_decode = false
  local from_c = req:getResult(from)[1]
  local to_c = req:getResult(to)[1]
...
--- ZhengsuData 整肃的数据
---@class ZhengsuDataSpec
---@field public player ServerPlayer @ 发起整肃的角色
---@field public target ServerPlayer @ 执行整肃的角色
---@field public skill_name string @ 整肃的技能
---@field public reward any @ 整肃的奖励（"draw2" | "recover"，可改为执行任意函数）

---@class Utility.ZhengsuData: ZhengsuDataSpec, TriggerData
Utility.ZhengsuData = TriggerData:subclass("ZhengsuData")

--- 整肃 TriggerEvent
---@class Utility.ZhengsuTE: TriggerEvent
---@field public data Utility.ZhengsuData
Utility.ZhengsuTE = TriggerEvent:subclass("ZhengsuEvent")

--- 整肃开始
---@class Utility.StartZhengsu: Utility.ZhengsuTE
Utility.StartZhengsu = Utility.ZhengsuTE:subclass("Utility.StartZhengsu")
--- 整肃奖励前
---@class Utility.ZhengsuRewarding: Utility.ZhengsuTE
Utility.ZhengsuRewarding = Utility.ZhengsuTE:subclass("Utility.ZhengsuRewarding")
--- 整肃奖励后
---@class Utility.ZhengsuRewarded: Utility.ZhengsuTE
Utility.ZhengsuRewarded = Utility.ZhengsuTE:subclass("Utility.ZhengsuRewarded")

---@alias ZhengsuTrigFunc fun(self: TriggerSkill, event: Utility.ZhengsuTE,
---  target: ServerPlayer, player: ServerPlayer, data: Utility.ZhengsuData):any

---@class SkillSkeleton
---@field public addEffect fun(self: SkillSkeleton, key: Utility.ZhengsuTE,
---  data: TrigSkelSpec<ZhengsuTrigFunc>, attr: TrigSkelAttribute?): SkillSkeleton

--- 整肃 GameEvent
Utility.ZhengsuEvent = "Zhengsu"
...
Utility.startZhengsu = function(player, target, skillName, prompt)
  ...
  room.logic:trigger(Utility.StartZhengsu, target, data)
  choice = data.choice
...
---@param reward any @ 要获得的奖励（默认"draw2"，可以改为执行任意函数）
---@param skillName string @ 技能名
--- 获得整肃奖励
Utility.rewardZhengsu = function(player, target, reward, skillName)
  ...
  room.logic:trigger(Utility.ZhengsuRewarding, target, data)
  reward = data.reward
  ...
  elseif type(reward) == "function" then
    reward()
  end
  room.logic:trigger(Utility.ZhengsuRewarded, target, data)
end
...
--- 从移动数据中获得一名角色失去的牌数。由于手杀服规，使用装备牌不算失去装备，需要排除
...
--- 从移动事件中获得一名角色失去的牌数。由于手杀服规，使用装备牌不算失去装备，需要排除
---@param player ServerPlayer
---@param event GameEvent.MoveCards
---@return integer[] @ 失去的牌
Utility.getLostCardsFromMoveEvent = function(player, event)
  local data = event.data
  local equipUsing = {}
  local parentUseData = event:findParent(GameEvent.UseCard)
  if parentUseData then
    local use = parentUseData.data
    if use.card.type == Card.TypeEquip and use.from == player then
      equipUsing = Card:getIdList(use.card)
